...
from utils.inference import detect_disease
from utils.inference import get_all_disease_classes
...
def load_disease_catalog():
    df = pd.read_csv("data/disease_catalog.csv")
    df["Disease Key"] = df.iloc[:, 0] + " " + df.iloc[:, 1]  # plant_type + disease
    return df
...
<div class='hero'>
    ...
    <p>Detect plant diseases and get treatment advice using deep learning</p>
</div>
""", unsafe_allow_html=True)

# ─── LAYOUT: Upload | Detect | Advice ─────────────────────────────────────
...
with col2:
    ...
    if "uploaded_image" in st.session_state:
        ...
        st.warning("Upload an image to detect.")
    st.markdown("</div>", unsafe_allow_html=True)

# 💡 Treatment Advice (from CSV)
with col3:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    ...
    if "detection_result" in st.session_state:
        ...
        if st.button("Generate Advice") and key:
            ...
            if not match.empty:
                ...
                st.warning(f"No record found for: {key}")
    else:
        st.warning("Detect disease first.")
...
with st.expander(" Full Disease Treatment Library"):
    ...
    if results.empty:
        st.info("No diseases found for your search.")
    else:
        for _, row in results.iterrows():
            st.subheader(f"🦠 {row['Disease Key']}")
...
with st.expander("🆘 Help & FAQ"):
    st.markdown("""
    **Q1: What types of plants does this support?**  
    ✅ Currently supports tomato, maize, cassava, and cashew.

    **Q2: How accurate is the detection?**  
    📸 Accuracy depends on image clarity and model training. Use high-resolution close-ups.

    **Q3: Who can I contact?**  
